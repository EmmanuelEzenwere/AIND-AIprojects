...
class AirCargoProblem(Problem):
    ...
    def get_actions(self):
            ...
            for cargo in self.cargos:
                for plane in self.planes:
                    for airport in self.airports:
                        ...
                        fluent = expr("Load({}, {}, {})".format(cargo, plane, airport))
                        preconds_pos = [expr("At({}, {})".format(cargo, airport)),
                                        expr("At({}, {})".format(plane, airport))]

                        preconds_neg = []

                        effects_add = [expr("In({}, {})".format(cargo, plane))]
                        effects_rem = [expr("At({}, {})".format(cargo, airport))]
        ...
        def unload_actions():
            ...
            for cargo in self.cargos:
                for plane in self.planes:
                    for airport in self.airports:
                        fluent = expr("Unload({}, {}, {})".format(cargo, plane, airport))
                        preconds_pos = [expr("In({}, {})".format(cargo, plane)),
                                        expr("At({}, {})".format(plane, airport))
                                        ]
                        preconds_neg = []
                        effects_add = [expr("At({}, {})".format(cargo, airport))]
    ...
    def actions(self, state: str) -> list:
        ...
        prop_kb.tell(sentence)
        for action in self.actions_list:
            is_possible = True

            for precond in action.precond_pos:
                if precond not in prop_kb.clauses:
                    is_possible = False

            for precond in action.precond_neg:
                if precond in prop_kb.clauses:
                    is_possible = False
...
def air_cargo_p2() -> AirCargoProblem:
    ...
    initial = FluentState(preconds_pos, preconds_neg)
...
def air_cargo_p3() -> AirCargoProblem:
    ...
    Init(At(C1, SFO) ∧ At(C2, JFK) ∧ At(C3, ATL) ∧ At(C4, ORD)∧ At(P1, SFO) ∧ At(P2, JFK)
                     ∧ Cargo(C1) ∧ Cargo(C2) ∧ Cargo(C3) ∧ Cargo(C4) ∧ Plane(P1) ∧ Plane(P2)

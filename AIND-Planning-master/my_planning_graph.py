...
class PgNode:
    ...
    includes instance sets common to both types of nodes(parents & children nodes) used in a planning graph.
...
class PlanningGraph:
    ...
    def add_literal_level(self, level):
        ...
        #  of all the A nodes that could produce them, and likewise add the A nodes to the parent sets of the S nodes.
    ...
    def update_a_mutex(self, nodeset):
        ...
        for i, n1 in enumerate(nodelist[:-1]):
            for n2 in nodelist[i + 1:]:
                # Boolean, tests if the planning graph is a serial planning graph...
                serial_planning_graph = self.serialize_actions(n1, n2)
                # Boolean, tests if the action pairs have Inconsistent Effects & Interference & Competing needs.
                pair_conditions = self.inconsistent_effects_mutex(n1, n2)\
                    or self.interference_mutex(n1, n2) \
                    or self.competing_needs_mutex(n1, n2)
                # If all the above conditions hold then we mutexify the action pairs.
                if serial_planning_graph or pair_conditions:
                    mutexify(n1, n2)
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        mutual_exclusion = (node_s1.symbol == node_s2.symbol) and (node_s1.is_pos != node_s2.is_pos)
